...
var prioritizeDependencies = function (config, fileType) {
    ...
    _.toArray(config.get('global-dependencies').get()).
      ...
      filter(function (dependency) {
        if (_.contains(eliteDependencies, dependency.name)) {
          eliteDependenciesCaught.push(dependency);
        } else {
          return true;
        }
      })
  ...
  eliteDependencies.reverse().
    forEach(function (dependency) {
      var eliteDependency =_.find(eliteDependenciesCaught, { name: dependency });
      if (eliteDependency) {
        dependencies.unshift(eliteDependency.main);
      }
    });
